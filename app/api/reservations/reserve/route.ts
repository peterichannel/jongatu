...
export async function POST(req: Request) {
  ...
  const { data, error } = await supabase.rpc('reserve_presentation_slot', {
    p_member_id: me.id,
  ...
  const result = data as { success: boolean; error?: string; transferred?: boolean; transferred_from?: string | null }
  if (!result.success) {
    return NextResponse.json({ error: result.error || '예약 실패' }, { status: 400 })
  }
