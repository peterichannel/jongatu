...
- 마감 = **스터디 시작(수 19:00 고정) 10분 전**. 시작 직전까지 답변받되 운영진 인원 파악 10분을 확보.
- `lib/seoul-time.ts` 에 `STUDY_START_MINUTES`(19:00)·`PRE_ATTENDANCE_CUTOFF_MINUTES`(18:50)·`formatMinutesKR()`·`isPreAttendanceOpen()` 추가. 시작 시각은 **상수 하드코딩**(옵션 A) — 회차마다 달라지면 그때 `sessions.start_time` 컬럼으로 확장.
- **API 가 실제로 마감을 막는다** — `POST /api/pre-attendance` 는 그동안 `session.date < today` (지난 회차)만 걸러 회차 당일 종일 답변/수정이 가능했다. 표시 문구(전날 자정)와 실제 규칙이 어긋나 있던 것을 `isPreAttendanceOpen()` 으로 일원화.
- 멤버 UI: 마감 후 `[변경]` 버튼이 "답변 마감"으로 바뀌고, 미응답자에겐 폼 대신 마감 안내. 마감 판정은 hydration 불일치를 피해 마운트 후 계산 + 30초 간격 갱신.
- 안내/재안내 메시지 문구: `답변 마감: 7월 22일 (수) 오후 6시 50분 (스터디 10분 전)` (홈 운영자 카드 + 관리자 사전참석 상세 양쪽).

### 사전참석 불참 시 사유 입력 필수화
- **서버 검증 추가** — 클라이언트엔 이미 빈 사유 가드가 있었지만 API 는 무방비여서 `status='absent', reason=null` 저장이 가능했다. 이제 400 `"불참 시 사유를 입력해주세요"`.
- 참석 선택 시 `reason` 은 서버에서 무조건 `null` 로 덮어씀 — 불참→참석 변경 시 이전 사유가 남지 않는다. 저장 시 100자로 절단.
- 입력 UI: 빠른 선택 칩 4종(회사 일정/가족 일정/몸이 안 좋음/출장/여행) + 직접 입력 textarea(min-h 60px, 16px, 100자 카운터), 에러 시 빨간 테두리, 저장 후 "✓ 사전참석 답변 저장되었습니다" 5초 배너.
- `pre_attendances.reason` 컬럼은 0001 초기 마이그레이션에 이미 존재 — 추가 마이그레이션 없음.

### 출석 체크 다시하기 (멤버)
- 체크인 후 상태 칩만 보이고 되돌릴 방법이 없던 것을 보완 — "체크 시각이 잘못됐나요?" → `[다시 체크]` 로 현재 시각 기준 재판정. 체크인 API 의 POST 가 이미 기존 행을 갱신하므로 신규 엔드포인트 없음.
- **체크 자체를 무르는 '취소'는 넣지 않음** — 미체크 = 결석이라 멤버가 스스로 결석 처리하는 꼴이 된다. 정정은 운영자 출석체크 화면에서.
- 운영자가 확정(`is_confirmed`)한 회차에서는 버튼이 뜨지 않는다.

### 사전참석 카드 노출 조건 개선 — 한 시점에 최대 하나
- `/attendance` 가 "오늘 회차"와 "다음 회차" 패널을 **동시에** 띄우던 것을 정리. 오늘(7/22) 회차가 있는데 D-14인 8/5 회차 사전참석까지 요구해 인지 부담이 컸고, 그 시점 답변은 어차피 대부분 번복됨.
- 노출 규칙: **오늘 회차가 있으면 다음 회차 패널은 숨김** / 오늘 회차가 없으면 다음 회차가 **D-3 이내**일 때만 노출 (`PRE_ATTEND_WINDOW_DAYS = 3`).
- 응답 창이 아직 안 열린 경우 "다가오는 회차가 없습니다"(오해 소지)가 뜨던 자리에, 다음 회차 날짜/회차번호와 **응답 시작일**을 알려주는 안내 + `[일정 보기]` 링크로 대체. 회차가 정말 없을 때의 기존 안내는 그대로 유지.
- 발표자 목록은 D-3에 패널이 열릴 때 기존 발표 섹션으로 함께 표시된다(추가 작업 없음).
- 홈(`app/(member)/page.tsx`)의 "사전참석 답하기" 액션 카드는 이미 D-2~D-1 로 더 좁게 게이팅돼 있어 변경하지 않음.

### 관리자 홈: 사전참석 상세 페이지 추가 (누가 답했는지 확인 가능)
- 관리자 홈 사전참석 카드에 `[상세 →]` 링크 추가 — "오늘 출석" 카드와 동일한 위치/스타일.
- 신규 `app/admin/(authed)/schedule/[sessionId]/pre-attendance/page.tsx` — 활성 멤버(`is_active=true`) 기준으로 **참석(녹색) / 불참(빨강) / 미응답(노랑)** 3그룹 명단. 이름 18px, 각 이름 옆에 응답 시각(KST `HH:MM 답변`), 불참은 사유 병기. 그룹 내 정렬은 `members.order('name')`(가나다순) 그대로.
- **재안내 메시지 복사** — 미응답자 이름이 박힌 단톡방 문구 + `CopyShareButtons`(복사/공유, 5초 "복사됨"). 마감일은 회차 전날 자정, URL 줄은 기존 컨벤션대로 `NEXT_PUBLIC_SITE_URL` 이 있을 때만 붙는다.
- **미응답자 이름만 복사** — `copy-names-button.tsx` (쉼표 구분, 클립보드 실패 시 textarea 폴백).

### 출석 상세: 상태별 명단 + 미체크자 안내 메시지
- 기존 `/admin/schedule/[sessionId]/attendance` 는 멤버별 체크 에디터일 뿐 **상태별로 누가 있는지 모아 보는 화면이 없었음** → 하단에 출석/지각/결석/공결/미체크 5그룹 명단 추가. 체크 즉시 반영(클라이언트 state 기준).
- 미체크 인원이 있을 때만 "출석체크 안내 메시지" 카드 + 복사/공유 버튼 노출.

### fix: 빌드 실패 복구
- `app/(member)/loading.tsx` 의 미사용 import(`SkeletonBar`)로 `next build` 가 lint 단계에서 깨져 있던 것 제거 (f9ae690 에서 유입).

### 앱 상단 헤더에 스터디 브랜딩 추가
- 공유 `components/BrandHeader.tsx` 신규 — 메인 타이틀 **"종로 가치 투자 스터디"** + 슬로건 **"우리는 부자다!"**, 가운데 정렬.
- 멤버 레이아웃(`app/(member)/layout.tsx`)·관리자 레이아웃(`app/admin/(authed)/layout.tsx`) 최상단에 배치 → 로그인 화면 포함 전 페이지 공통 노출.
- 홈(`app/(member)/page.tsx`)의 중복 h1("종로 가치 투자 스터디")은 제거하고 레이아웃 헤더로 일원화 (인사말 "안녕하세요 {이름}님"은 콘텐츠 상단 그대로 — Option A). 홈 `loading.tsx`의 타이틀 스켈레톤도 제거(레이아웃이 즉시 렌더).
- 타이틀 20px/bold/gray-900, 슬로건 14px/green-600(브랜드 톤). 인쇄 시 숨김(`print:hidden`, 재무 리포트 자체 헤더 보존).

## 2026-07-21

### 초기 로딩 성능 개선 — 서버 쿼리 병렬화 + 스켈레톤 보강
- **perf: getAuthedMember/getAuthedAdmin `cache()` 래핑** — 같은 요청 안에서 layout·page 가 각각 인증을 확인해도 `members` 조회는 1회만. 관리자 경로에서 왕복 1회 절약.
- **perf: 페이지별 쿼리 병렬화** — 독립 쿼리를 `Promise.all` 로 묶어 "파(wave)" 단위로 재구성. 의존 관계가 있는 쿼리(A 결과로 B 조회)는 순차 유지.
  - 홈: 직렬 13회 → 3파 / `/me`: 7회 → 3파 / `/schedule`: → 3파 (`force-dynamic` 은 `revalidate=0` 과 중복이라 제거)
  - `/attendance`·`/evaluation`: 인증+활성분기+명단을 1파로 통합 / `/admin`·`/admin/finance`·`/admin/evaluations`: 3파
- **feat: loading.tsx 일괄 추가** (멤버 5 + 관리자 6, 공용 `components/Skeleton.tsx`). 관리자 영역은 그동안 loading.tsx 가 전혀 없어 흰 화면이었음 — `app/admin/loading.tsx` 가 레이아웃의 운영자 인증 대기 구간까지 덮는다.
- **성능 개선 결과** (로컬 프로덕션 빌드 → Supabase Seoul, 로그인 상태, 응답 완료 시각 중앙값 7회):

  | 페이지 | 전 | 후 | 개선 |
  |---|---|---|---|
  | 홈 `/` | 240ms | 80ms | **-67%** |
  | `/me` | 147ms | 71ms | -52% |
  | `/admin` | 148ms | 100ms | -32% |
  | `/schedule` | 95ms | 67ms | -29% |

  - 참고: 같은 환경에서 Supabase 쿼리 4건 직렬 73ms vs 병렬 23ms — 개선분은 대부분 네트워크 왕복 제거에서 나옴.
  - TTFB 는 스켈레톤 껍데기가 스트리밍되는 시점이라 이미 ~12ms 로 측정돼 지표로 부적합. 위 표는 **콘텐츠 완료 시각** 기준.
- 범위 밖(V2 백로그): 응답 캐싱, `revalidate` 재설계.

### Supabase 리전 Singapore → Seoul 이전 (실사용자 데이터 없어 시드 초기화)
- DB 리전을 Northeast Asia (Seoul, ap-northeast-2) 로 이전. 실사용자 데이터가 없어 이관 없이 새 프로젝트로 완전 재시작.
- 스키마 재구축: 마이그레이션 0001~0017 + 0019 순서 실행. `0018_seed_2026_h1_attendance`(과거 출결 백필)는 새 시작이므로 **제외**.
- RPC(`reserve_presentation_slot`/`cancel_presentation_slot`/`update_presentation_company`)·분기당 1인 1회 발표 제약·과거 회차 예약 가드 정상 이관 확인.
- `.env.local` Seoul URL/anon/service 키로 교체. 로컬 검증: 홈 멤버 17명 렌더·에러 0·웜 응답 ~0.14s.

### 26-Q2부터 새로 시작, 이전 시드 데이터 폐기
- 최소 기본 시드만 입력: 활성 멤버 17명(앱 관리자 이상호·양민기·이우재 3명 + 일반 14명), 반기 `2026-H1`(완료)/`2026-H2`(활성), 페널티 규칙 4종(사전참석 미등록은 `is_active=false`).
- 발표 분기는 26-Q2가 오늘(7/21) 기준 이미 종료(마지막 7/8)라 과거 회차 예약 가드에 막히므로, 활성 반기 H2에 맞춰 **`2026-Q3`(활성)** 로 신설 — 미래 격주 수요일 6회차 × 빈 슬롯 3개로 즉시 예약 가능.
- 과거 출결·보증금 잔액·거래내역·사전참석·평가·예약 로그는 시드하지 않음 (실제 운영에서 발생).

### 26-H2 시작 (실전 앱 사용 개시) — 7/22 첫 스터디
- 앱 실사용 개시 시점 데이터 세팅. 활성 반기 `2026-H2`(is_active), 활성 분기 `2026-Q3`(2026-07-22~09-30) 확정.
- **26-H1 데이터는 외부에서 수동 정산 완료**되어 앱에 미반영 (보증금·출결·정산 이력 없음).
- 26-H2 보증금 초기화: 활성 멤버 15명 × 45,000원(총 675,000) + 초기 입금 기록. 운영비 13명 × 45,000원(총 585,000, 운영진 양민기·이우재 면제).
- 활성 멤버 15명 기준 — 임지향·이애리는 **26-H1 종료 후 탈퇴**로 `is_active=false` 처리(이력은 보존), H2 세팅 대상 제외. 두 명의 H2 보증금·운영비·Q3 예약 슬롯 없음 확인.

### 26-Q3 6회차 등록, 12명 확정 + 6슬롯 선착순 오픈
- 회차 6개(격주 수, 7/22·8/5·8/19·9/2·9/16·9/30) × 슬롯 3개 = 18슬롯.
- 확정 예약 12건 관리자 대리 입력: Q3-1 박성철('26 상반기 포트폴리오 반성')·김동주(TKG휴켐스 or 파인엠텍)·유병우, Q3-2 박경환·이상호·김윤정, Q3-5 서수민·김영호·이종명, Q3-6 강성민·최수진·서유나.
- Q3-3·Q3-4 6슬롯은 선착순 오픈. 확정 12명 전원 distinct라 분기당 1인1회 트리거 위반 없음.
